...
import { param } from 'express-validator'; // Para validar UUID en rutas
...
/**
 * Valida que un parámetro de ruta (URL) sea un UUID válido.
 * @param {string} nombreParametro - El nombre del parámetro en la URL (ej: 'entrenadorID', 'entrenamientoId').
 * @param {string} [mensajePersonalizado] - Mensaje de error opcional.
 * @returns Un array de middleware de validación para express-validator.
 */
export const validarParametroUUID = (nombreParametro, mensajePersonalizado) => {
  const mensaje = mensajePersonalizado || `El ID '${nombreParametro}' en la URL debe ser un UUID válido.`;
  return [ // Retorna un array, como espera express-validator
